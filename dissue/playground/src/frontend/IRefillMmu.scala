...
import top.frontend.bundle.{ICacheRefillReq, ICacheRefillResp}
...
class ICacheRefillMmu(
  ...
  private val exceptionReg = Reg(new top.bundle.FetchException(cacheCfg.addrWidth))
  ...
  private def toFetchException(info: ExceptionInfo): top.bundle.FetchException = {
    val out = Wire(new top.bundle.FetchException(cacheCfg.addrWidth))
