...
import top.core.backend.bundle.{BpuPerfSample, RetireGroup}
...
class DifftestBridge(cfg: BackendConfig = BackendConfig(), enableDpi: Boolean = true) extends Module {
...
class BpuPerfBridge(enableDpi: Boolean = true) extends Module {
  val io = IO(new Bundle {
    val sample = Input(new BpuPerfSample)
  })

  if (enableDpi) {
    NpcBpuPerf.callWithEnable(
      io.sample.valid,
      io.sample.cfiClass.pad(32),
      io.sample.predHit,
      io.sample.predTaken,
      io.sample.actualTaken,
      io.sample.correct
    )
  }
}
