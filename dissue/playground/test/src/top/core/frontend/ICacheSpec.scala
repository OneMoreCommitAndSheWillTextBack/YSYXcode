...
object ICacheSpec {
  ...
  def main(args: Array[String]): Unit = {
    simulate(new ICache(cfg)) { dut =>
      ...
      // A replayed miss is probed on the older refill's completion edge, so its refill request follows immediately.
      driveRequest(valid = true, token = 9, addr = 0x4000)
      dut.io.req.ready.expect(true)
      dut.clock.step()
      driveRequest(valid = true, token = 10, addr = 0x5000)
      dut.io.lookup.valid.expect(true)
      ...
      dut.io.refillReq.valid.expect(true)
      dut.io.refillReq.bits.addr.expect(0x4000)
      dut.clock.step()
      driveRefill(valid = true, data = line0)
      dut.io.lookup.valid.expect(true)
      dut.io.lookup.bits.token.tag.index.expect(10 % cfg.ftqEntries)
      dut.io.lookup.bits.missingMask.expect(1)
      dut.clock.step()
      driveRefill(valid = false)

      dut.io.resp.valid.expect(true)
      dut.io.resp.bits.token.tag.index.expect(9  % cfg.ftqEntries)
      dut.io.refillReq.valid.expect(true)
      dut.io.refillReq.bits.addr.expect(0x5000)
      dut.clock.step()
      driveRefill(valid = true, data = line1)
      dut.io.refillResp.ready.expect(true)
      dut.clock.step()
      driveRefill(valid = false)
      dut.io.resp.valid.expect(true)
      dut.io.resp.bits.token.tag.index.expect(10 % cfg.ftqEntries)
      dut.clock.step()

      // With no replay buffered, a request waiting on the final refill is accepted and probed on that same cycle.
      driveRequest(valid = true, token = 11, addr = 0x6000)
      dut.io.req.ready.expect(true)
      dut.clock.step()
      driveRequest(valid = false)
      dut.io.lookup.valid.expect(true)
      ...
      dut.clock.step()
      dut.io.refillReq.valid.expect(true)
      dut.io.refillReq.bits.addr.expect(0x6000)
      dut.clock.step()

      driveRefill(valid = true, data = line0)
      driveRequest(valid = true, token = 12, addr = 0x7000)
      dut.io.req.ready.expect(true)
      dut.io.lookup.valid.expect(true)
      dut.io.lookup.bits.token.tag.index.expect(12 % cfg.ftqEntries)
      dut.io.lookup.bits.missingMask.expect(1)
      dut.clock.step()
      driveRefill(valid = false)
      driveRequest(valid = false)
      ...
      dut.io.resp.valid.expect(true)
      dut.io.resp.bits.token.tag.index.expect(11 % cfg.ftqEntries)
      dut.io.refillReq.valid.expect(true)
      dut.io.refillReq.bits.addr.expect(0x7000)
      dut.clock.step()
      driveRefill(valid = true, data = line1)
      dut.clock.step()
      driveRefill(valid = false)
      dut.io.resp.valid.expect(true)
      dut.io.resp.bits.token.tag.index.expect(12 % cfg.ftqEntries)
      dut.clock.step()
