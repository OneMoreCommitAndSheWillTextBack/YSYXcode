...
static unsigned lwlr_ans[] = {
    0xbc025800, 0x007fff4a, 0xcd810080, 0x00ffffab};

int main(void) {
  for (int i = 0; i < LENGTH(mem); i++) {
    check((short)mem[i] == (short)lh_ans[i]);
  }
  ...
  for (int i = 0; i < ((LENGTH(mem) / 2) - 1); i++) {
    unsigned x = am_load_u32_unaligned(bytes + 1 + (i << 2));
    check(x == lwlr_ans[i]);
  }

  for (int i = 0; i < LENGTH(mem); i++) {
    mem[i] = (unsigned short)~(1u << ((i << 1) + 1));
